...
import { Zap, ArrowRight, Loader2, CheckCircle2, TrendingUp, Clock, Target, ShieldCheck, Sparkles, Trophy } from 'lucide-react';
        ...
        <div className="w-full max-w-2xl space-y-8 relative z-10">
          <div className="text-center space-y-4">
            ...
            <h1 className="text-5xl md:text-7xl font-black italic uppercase tracking-tighter leading-[0.85]">
              PLANO GERADO <br /><span className="text-primary shimmer-text">PARA VOCÊ</span>
            </h1>
          </div>

          <Card className="glass-card border-primary/30 rounded-[2.5rem] overflow-hidden shadow-[0_0_60px_rgba(139,92,246,0.15)]">
            <CardHeader className="bg-primary/5 border-b border-white/5 p-8 flex flex-row items-center justify-between">
              ...
              <div className="h-10 w-10 relative opacity-50">
                <Image src={LOGO_ICON} alt="Icon" fill className="object-contain" />
              </div>
            </CardHeader>
            <CardContent className="p-8 space-y-10">
              <div className="space-y-4">
                <p className="text-muted-foreground uppercase text-[10px] font-black tracking-[0.3em] opacity-50">Seu Plano de Ataque Neural</p>
                <div className="space-y-3">
                  <h3 className="text-3xl md:text-4xl font-black italic text-white leading-none tracking-tight">
                    {aiPlan?.[0]?.title || "Estratégia de Escala Flow"}
                  </h3>
                  <p className="text-sm text-muted-foreground font-medium leading-relaxed italic border-l-2 border-primary/40 pl-4 py-1">
                    "{aiPlan?.[0]?.description || "Otimização de processos e automação de abordagem para máxima conversão."}"
                  </p>
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-6 rounded-2xl bg-white/[0.03] border border-white/5 group hover:border-primary/30 transition-all">
                  <div className="flex items-center gap-2 mb-3 text-primary/70">
                    <Clock className="h-4 w-4" />
                    <span className="text-[10px] font-black uppercase tracking-widest">Esforço Diário</span>
                  </div>
                  <p className="text-2xl font-black italic text-white">{answers.hours} <span className="text-xs text-muted-foreground not-italic font-medium">p/ dia</span></p>
                </div>
                <div className="p-6 rounded-2xl bg-primary/5 border border-primary/20 relative overflow-hidden group">
                  ...
                  <div className="flex items-center gap-2 mb-3 text-primary">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest">Meta Projetada</span>
                  </div>
                  <p className="text-4xl font-black italic text-white tracking-tighter">R$ {answers.target[0] || answers.target}</p>
                  <p className="text-[9px] font-bold text-primary/60 uppercase tracking-widest mt-1">Ganhos em 30 Dias</p>
                </div>
              </div>

              <div className="p-8 rounded-3xl bg-white/5 border border-dashed border-white/10 text-center relative overflow-hidden">
                <div className="absolute inset-0 bg-primary/5 blur-3xl rounded-full opacity-50"></div>
                <Sparkles className="h-6 w-6 text-primary mx-auto mb-3 relative z-10" />
                <p className="text-xs font-black uppercase tracking-widest text-white mb-2 relative z-10">Passo 1 de 7 já desbloqueado</p>
                <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest max-w-xs mx-auto leading-relaxed relative z-10">
                  O restante do seu plano neural está pronto e aguardando liberação na área do aluno.
                </p>
              </div>
              ...
              <Button 
                ...
                className="w-full h-24 rounded-[2rem] bg-primary text-2xl font-black uppercase tracking-widest shadow-[0_20px_50px_rgba(139,92,246,0.3)] hover:scale-[1.03] active:scale-[0.98] transition-all group"
              >
                ATIVAR MEU ACESSO <ArrowRight className="ml-3 h-8 w-8 group-hover:translate-x-2 transition-transform" />
              </Button>
            </CardContent>
          </Card>
          
          <p className="text-center text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground animate-pulse">
            Neural Sync Complete • 100% Personalizado
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#050508] flex flex-col items-center justify-center p-4">
      <div className="w-full max-xl space-y-8 relative z-10 animate-in fade-in duration-500">
        <div className="text-center space-y-2">
          <div className="relative h-12 w-12 mx-auto mb-4 grayscale hover:grayscale-0 transition-all">
             <Image src={LOGO_ICON} alt="Icon" fill className="object-contain" />
          </div>
        ...
        <Card className="glass-card border-white/10 rounded-[2.5rem] overflow-hidden">
            ...
            {currentStepData.type === 'radio' && (
              ...
              >
                {currentStepData.options?.map((opt) => (
                  <Label
                    key={opt}
                    className={`flex items-center justify-between p-6 rounded-2xl border-2 cursor-pointer transition-all ${answers[currentStepData.id] === opt ? 'border-primary bg-primary/10 text-white shadow-[0_0_30px_rgba(139,92,246,0.1)]' : 'border-white/5 bg-white/[0.02] hover:bg-white/5 text-muted-foreground'}`}
                  >
            ...
            {currentStepData.type === 'slider' && (
              <div className="space-y-8 py-4">
                <div className="text-center">
                  <span className="text-5xl font-black italic text-primary tracking-tighter">R$ {answers.target[0] || answers.target}</span>
                  <p className="text-[10px] font-black uppercase text-muted-foreground tracking-[0.3em] mt-2">Expectativa de faturamento mensal</p>
                </div>
                <Slider
                  defaultValue={[1500]}
                  max={5000}
            ...
            {currentStepData.type === 'text' && (
              <div className="py-4">
                <Input 
                  ...
                  className="bg-white/5 border-white/10 h-20 rounded-2xl text-lg font-medium focus-visible:ring-primary px-6"
                  value={answers[currentStepData.id] || ''}
          ...
          <div className="px-8 pb-8 flex gap-3">
            <Button 
              ...
              className="flex-1 h-16 border-white/10 rounded-2xl font-black uppercase tracking-widest hover:bg-white/5 text-[10px]"
            >
              VOLTAR
            </Button>
            <Button 
              ...
              className="flex-[2] h-16 bg-primary hover:bg-primary/90 rounded-2xl font-black uppercase tracking-widest shadow-lg shadow-primary/20 text-[10px]"
            >
              {currentStep === STEPS.length - 1 ? "GERAR MEU PLANO NEURAL" : "PRÓXIMO"} 
